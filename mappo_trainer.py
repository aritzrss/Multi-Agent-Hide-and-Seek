        ...
        for epoch in range(self.n_epochs):
            ...
            for start_idx in range(0, len(indices), self.batch_size):
                ...
                for idx in batch_indices:
                    ...
                    with torch.no_grad():
                        hider_features_detached = self.agent.get_features(obs, "hider", self.env)
                    hider_features = self.agent.get_features(obs, "hider", self.env)
                    
                    # Handle inactive seeker
                    if obs["seeker"]["state"][0] < 0:
                        with torch.no_grad():
                            seeker_features_detached = torch.zeros_like(hider_features)
                        seeker_features = torch.zeros_like(hider_features).detach()
                        seeker_active = False
                    else:
                        with torch.no_grad():
                            seeker_features_detached = self.agent.get_features(obs, "seeker", self.env)
                    ...
                    for agent_key in ["hider", "seeker"]:
                        ...
                        action_tensor = torch.tensor(action, dtype=torch.long, device=self.device, requires_grad=False)
                        current_log_prob = dist.log_prob(action_tensor)
                        entropy = dist.entropy()
                        
                        # Old log prob (detach to avoid computational graph issues)
                        old_log_prob = data["log_probs"][agent_key][idx].detach()
                        
                        # Ratio and clipped objective
                        ratio = torch.exp(current_log_prob - old_log_prob)
                        adv = torch.tensor(advantages[agent_key][idx], dtype=torch.float32, device=self.device, requires_grad=False)
                    ...
                    target_return_value = (returns_tensor["hider"][idx].item() + returns_tensor["seeker"][idx].item()) / 2
                    target_return = torch.tensor(target_return_value, dtype=torch.float32, device=self.device, requires_grad=False)
                    value_loss = (current_value - target_return).pow(2)
